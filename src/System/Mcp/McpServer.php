 ...
 *
 * This is S2 from docs/zh/design/observability-automation.md: a protocol layer
 * on top of the S1 automation server. It ONLY consumes the §4 contracts —
 * the SemanticsNode tree (via rootsProvider), app state (via stateProvider), and
 * the drive handler — and exposes them as standard MCP tools / resources. It
 * never reaches into widget internals, keeping the agent decoupled from whether
 * a node is a libui native control or a self-drawn Surface widget.
 *
 * The transport is deliberately the JSON-RPC 2.0 subset used by the MCP
 * "Streamable HTTP" transport: a single POST per request carrying a JSON-RPC
 * envelope and returning a JSON-RPC envelope. No SSE streaming is required
 * because all tools are request/response.
 *
 ...
 * without sockets or FFI. The S1 AutomationServer simply mounts it on /mcp.
 */
final class McpServer
{
    ...
    /**
     * @param callable(): iterable         $rootsProvider Returns roots (SemanticsNode | SemanticProvider) for the snapshot.
    ...
    private function dispatchOne(mixed $msg): ?array
    {
        ...
        if ($method === 'initialize') {
            return $this->result($id, [
                ...
                'instructions' => 'This server exposes the running UI as an accessibility '
                    ...
                    . 'action (ui_drive). Node ids come from ui_snapshot. Open GET /mcp as an '
                    . 'SSE stream to receive live server notifications: '
                    . 'notifications/resources/updated (ui://snapshot) and '
                    . 'notifications/state_changed (the new state) after every state transition.',
            ]);
        }
        ...
        // Notifications (no id, incl. notifications/initialized) get no response.
        if ($isNotification) {
            return null;
        }
    ...
    private function callTool(string $name, array $args): array
    {
        ...
        try {
            ...
            Log::error("MCP tool {$name} failed: " . $e->getMessage());
        ...
        // Tool convention: a payload carrying an `error` key is surfaced to the
        // MCP client as a tool error rather than a successful (but empty) result.
        if (array_key_exists('error', $payload)) {
            return $this->toolError((string) $payload['error']);
        }
     ...
     * live snapshot resource. A client that receives this should re-`resources/read`
     * `ui://snapshot`. Consumes the §4 snapshot contract.
     *
     ...
     * new application state. Consumes the §4 state contract.
     *
    ...
    private function buildSnapshot(): array
    {
        ...
        foreach (($this->rootsProvider)() as $root) {
            $node = $root instanceof SemanticsNode ? $root
                : ($root instanceof SemanticProvider ? $root->semantics() : null);
            if ($node !== null) {
                $out[] = $node->toArray();
            }
        }
    ...
    private function registerBuiltins(): void
    {
        ...
        $this->tools['ui_snapshot'] = new Tool(
            'ui_snapshot',
            'Read the live accessibility tree (semantic nodes) of the running UI. '
        ...
        $this->tools['ui_get_state'] = new Tool(
            ...
            function (array $args): array {
                if ($this->stateProvider === null) {
                    return ['error' => 'no state provider registered'];
                }
                $state = ($this->stateProvider)();

                return $state === null ? ['error' => 'no state'] : $state;
            },
        );
        ...
        $this->tools['ui_drive'] = new Tool(
            ...
            [
                ...
                'properties' => [
                    ...
                    'payload' => [
                        'type' => 'object',
                        'description' => 'Optional extra arguments forwarded to the drive handler.',
