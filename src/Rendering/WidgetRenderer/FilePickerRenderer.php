...
/**
 * Self-drawn file picker: a read-only field (surface fill + primary/track
 * border) showing the selected path or a placeholder, with a trailing
 * downward chevron. Pressing it is handled at the Surface layer via
 * onClick(), which should open FilePickerDialog::pick().
 */
final class FilePickerRenderer implements WidgetRenderer
{
    use TokenWash;
    ...
    {
        if (! $spec instanceof FilePickerSpec) {
            throw new \InvalidArgumentException('FilePickerRenderer requires a FilePickerSpec');
        }
    ...
    private function paint(FilePickerSpec $spec, DesignTokens $tokens, string $path): Color
    {
        $c = $tokens->color($path);
