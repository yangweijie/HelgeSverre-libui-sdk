...
/**
 * Self-drawn date picker: a read-only field (surface fill + primary/track
 * border) showing the selected date or a placeholder, with a trailing
 * downward chevron. Pressing it is handled at the Surface layer via
 * onClick(), which should open DatePickerDialog::pick().
 */
final class DatePickerRenderer implements WidgetRenderer
{
    use TokenWash;
    ...
    {
        if (! $spec instanceof DatePickerSpec) {
            throw new \InvalidArgumentException('DatePickerRenderer requires a DatePickerSpec');
        }

        $bg = $spec->pressed ? $tokens->color('color.selection') : $tokens->color('color.surface');
        $border = $spec->focused
    ...
    public function render(WidgetSpec $spec, DesignTokens $tokens, float $width, float $height): RenderCommandList
    {
        ...
        // Trailing chevron (downward "v") on the right edge.
        $cx = $width - 12;
        $cy = $height / 2;
        $chevron = $spec->enabled
            ? Color::rgba($onSurface->r, $onSurface->g, $onSurface->b, 0.6)
            : Color::rgba($onSurface->r, $onSurface->g, $onSurface->b, 0.3);
        $commands[] = new StrokeLine($cx - 6, $cy - 4, $cx, $cy + 2, $chevron, 1.5);
        $commands[] = new StrokeLine($cx, $cy + 2, $cx + 6, $cy - 4, $chevron, 1.5);

        return new RenderCommandList($commands);
    }

    private function paint(DatePickerSpec $spec, DesignTokens $tokens, string $path): Color
    {
        $c = $tokens->color($path);
