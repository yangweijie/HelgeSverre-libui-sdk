...
/**
 * Self-drawn numeric field: identical geometry to {@see TextFieldRenderer},
 * with numeric bounds carried in the spec as hints for the host control.
 */
final class NumberRenderer implements WidgetRenderer
{
    use TokenWash;
    ...
    public function shapeCommands(WidgetSpec $spec, DesignTokens $tokens, float $width, float $height): array
    {
        if (! $spec instanceof NumberSpec) {
            throw new \InvalidArgumentException('NumberRenderer requires a NumberSpec');
        }
    ...
    private function paint(NumberSpec $spec, DesignTokens $tokens, string $path): Color
    {
        $c = $tokens->color($path);
