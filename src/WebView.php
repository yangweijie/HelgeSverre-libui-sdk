...
class WebView
{
     ...
     *
     * CAUTION: libui's uiWindowOnClosing only supports ONE callback — calling
     * this method will supersede any previously registered onClosing handler.
     * If you need both cleanup and Ffi::quit(), either:
     *   - Call this FIRST, then register your onClosing handler (yours wins), or
     *   - Skip this method and call destroy() manually after Loop::run() returns.
     *
     * The recommended pattern for avoiding crashes is to NOT use this method
     * and instead destroy the WebView after the event loop exits:
     *
     *     $window->onClosing(fn () => (Ffi::quit(), true));
     *     Loop::run();
     *     $webview->destroy();  // safe — NSApplication run loop is gone
     *
     * Destroying the WebView inside the onClosing callback (while the
     * NSApplication run loop is still active) can cause EXC_BAD_ACCESS
     * because the WKWebView's autoreleased Objective-C objects are still
     * in the run loop's autorelease pool when it drains.
     *
     * @return $this
     */
